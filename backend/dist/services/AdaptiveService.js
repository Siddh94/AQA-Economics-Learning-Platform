...
class AdaptiveService {
    constructor() {
        ...
        this.progressRepository = database_1.AppDataSource.getRepository(models_1.UserProgress);
    }
    /**
     * Calculate new difficulty level based on session score
     */
    calculateNewDifficultyLevel(currentLevel, score) {
        if (score >= 80) {
     ...
     */
    async getQuestionsForSession(difficultyLevel, count = 10) {
     ...
     */
    async updateUserProgress(userId, questions, answers) {
        const topicStats = {};
     ...
     */
    async getRecentSessionScores(userId, limit = 10) {
     ...
     */
    async updateUserDifficultyLevel(userId, newLevel) {
